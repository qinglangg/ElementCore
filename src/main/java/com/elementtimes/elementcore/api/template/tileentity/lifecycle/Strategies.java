...
/**
 * @see com.elementtimes.elementcore.api.template.tileentity.lifecycle.EnergyGeneratorLifecycle.IStrategy
 */
public class Strategies {
     ...
     * 仅向 East 方向发送
     */
    public static EnergyGeneratorLifecycle.IStrategy EAST = context -> sendTo(getSource(context, EnumFacing.EAST), getTarget(context, EnumFacing.EAST, EnumFacing.WEST));

    /**
     * 仅向 West 方向发送
     */
    public static EnergyGeneratorLifecycle.IStrategy WEST = context -> sendTo(getSource(context, EnumFacing.WEST), getTarget(context, EnumFacing.WEST, EnumFacing.EAST));

    /**
     * 仅向 North 方向发送
     */
    public static EnergyGeneratorLifecycle.IStrategy NORTH = context -> sendTo(getSource(context, EnumFacing.NORTH), getTarget(context, EnumFacing.NORTH, EnumFacing.SOUTH));

    /**
     * 仅向 South 方向发送
     */
    public static EnergyGeneratorLifecycle.IStrategy SOUTH = context -> sendTo(getSource(context, EnumFacing.SOUTH), getTarget(context, EnumFacing.SOUTH, EnumFacing.NORTH));
     ...
     */
    private static IEnergyStorage getSource(EnergyGeneratorLifecycle<? extends TileEntity> context, EnumFacing facing) {
     ...
     * @return 输出目标
     */
    private static IEnergyStorage getTarget(EnergyGeneratorLifecycle<? extends TileEntity> context, EnumFacing facing) {
     ...
     * @return 输出目标
     */
    private static IEnergyStorage getTarget(EnergyGeneratorLifecycle<? extends TileEntity> context, EnumFacing facing, EnumFacing opposite) {
        TileEntity te = context.getTe();
        if (!te.hasWorld()) {
            return null;
        }

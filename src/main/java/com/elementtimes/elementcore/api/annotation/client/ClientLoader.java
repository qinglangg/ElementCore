...
import com.elementtimes.elementcore.api.annotation.common.LoaderHelper;
...
public class ClientLoader {
            ...
            for (ASMDataTable.ASMData asmData : asmStateMapperData) {
                LoaderHelper.getBlock(elements, asmData.getClassName(), asmData.getObjectName()).ifPresent(block -> {
                    ...
                    LoaderHelper.getOrLoadClass(elements, propertyIn).ifPresent(clazz -> {
                        ...
                        elements.blockStateMaps.put(block, builder.build());
                    });
                });
            }
        }
        ...
        if (customDataSet != null) {
            for (ASMDataTable.ASMData asmData : customDataSet) {
                LoaderHelper.getBlock(elements, asmData.getClassName(), asmData.getObjectName()).ifPresent(block -> {
                    ...
                    elements.blockStateMaps.put(block, mapper);
                });
            }
        }

        elements.container.warn("loadBlockStateMapper: {}", elements.blockStateMaps.size());
    }
            ...
            LoaderHelper.getBlock(elements, asmData.getClassName(), asmData.getObjectName()).ifPresent(block -> {
                ...
                if (hasSubModel) {
                    ...
                    if (index > 0) {
                        ...
                        elements.blockStates.put(block, blockStateTriples);
                    }
                }
            });
        }
        elements.container.warn("loadBlockState: {}", elements.blockStates.size());
    }
    ...
    private static void loadBlockTesr(ECModElements elements) {
                    ...
                    if (pair != null) {
                        ...
                        if (animTesrClassName != null && !animTesrClassName.isEmpty()) {
                            ...
                            if (optional.isPresent()) {
                                elements.blockTesr.put(pair.right, optional.get());
                            } else {
                                elements.container.warn("Can't create AnimationTESR object");
                            }
                        } else {
                            elements.blockTesr.put(pair.right, new AnimationTESR());
                        }
                    } else {
        ...
        elements.container.warn("loadBlockTESR: {}", elements.blockTesr.size());
    }
        ...
        for (ASMDataTable.ASMData asmData : asmDataSet) {
            LoaderHelper.getBlock(elements,asmData.getClassName(), asmData.getObjectName()).ifPresent(block -> {
                ...
                if (!TextUtils.isBlank(colorClass)) {
                    IBlockColor color = elements.blockColorMap.get(colorClass);
                    if (color == null) {
                        ...
                        if (iBlockColorOpt.isPresent()) {
                            ...
                            elements.blockColorMap.put(colorClass, iBlockColor);
                            elements.blockColors.put(iBlockColor, new LinkedList<>());
                            color = iBlockColor;
                        }
                    }
                    if (color != null) {
                        elements.blockColors.get(color).add(block);
                    }
                }

                final String itemColorClass = (String) asmData.getAnnotationInfo().get("blockItemColorClass");
                if (!TextUtils.isBlank(itemColorClass)) {
                    IItemColor color = elements.blockItemColorMap.get(colorClass);
                    if (color == null) {
                        ...
                        if (iItemColorOpt.isPresent()) {
                            ...
                            elements.blockItemColorMap.put(colorClass, iItemColor);
                            elements.blockItemColors.put(iItemColor, new LinkedList<>());
                            color = iItemColor;
                        }
                    }
                    if (color != null) {
                        elements.blockItemColors.get(color).add(block);
                    }
                }
            });
        }
            ...
            LoaderHelper.getItem(elements, asmData.getClassName(), asmData.getObjectName()).ifPresent(item -> {
                ...
                if (metadatas != null || models != null) {
                    ...
                    if (index > 0) {
                        ...
                        elements.itemSubModel.put(item, modelTriples);
                    }
                }
            });
        }

        elements.container.warn("loadItemSub: {}", elements.itemSubModel.size());
    }
    ...
    private static void loadItemColor(ECModElements elements) {
        ...
        for (ASMDataTable.ASMData asmData : asmDataSet) {
            LoaderHelper.getItem(elements, asmData.getClassName(), asmData.getObjectName()).ifPresent(item -> {
                ...
                if (!TextUtils.isBlank(colorClass)) {
                    IItemColor color = elements.itemColorMap.get(colorClass);
                    if (color == null) {
                        final Optional<IItemColor> iItemColorOpt = ECUtils.reflect.create(colorClass, IItemColor.class, elements.container.logger);
                        if (iItemColorOpt.isPresent()) {
                            final IItemColor iItemColor = iItemColorOpt.get();
                            elements.itemColorMap.put(colorClass, iItemColor);
                            elements.itemColors.put(iItemColor, new LinkedList<>());
                            color = iItemColor;
                        }
                    }
                    if (color != null) {
                        elements.itemColors.get(color).add(item);
                    }
                }
            });
        }
        ...
        for (ASMDataTable.ASMData asmData : asmDataSet) {
            LoaderHelper.getItem(elements, asmData.getClassName(), asmData.getObjectName()).ifPresent(item -> {
                ...
                ECUtils.reflect.create(clazz, ItemMeshDefinition.class, elements.container.logger)
                        .ifPresent(meshDefinition -> elements.itemMeshDefinition.put(item, meshDefinition));
            });
        }
    }
}